...
contract OutlandsShards is PausableCPX {
    ...
    */
    ...
    /*
        external function for adding a shard of a specific period 
    */
    
    //useful for adding a current shard 
    function addShardOfPeriod (uint256 pid, uint256 pOfi, uint256 j, address player) 
        public
        returns (uint256 id)
    {
        require(hasRole(MINTER_ROLE, msg.sender), "No permission.");
    ...
    /*
        Using tokens to claim sh    ards 
    */
    uint256[3] internal _price = [201, 100, 300]; 
    
    function setPrice (uint256[3] calldata _p) 
        public
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "No permission.");
        _price = _p;
    ...
    function claimShardOfPeriod (uint256 pid, uint256 pOfi, uint256 j) 
        public
        returns (uint256)
    {
        //get period 
        uint256[] memory cp = getCurrentPeriod();
        
        //determine cost if period is current 
        uint256 cost = cp[pid] == pOfi ? _price[1] : _price[2]; 
        
        //burn tokens 
        GK.burn(msg.sender, _price[0], cost);
    ...
    function claimShardOfRegion (uint256 r) 
        public
        returns (uint256)
    {
        //burn tokens 
        GK.burn(msg.sender, _price[0], _price[2]);
